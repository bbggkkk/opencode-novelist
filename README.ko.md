...
**작품 레벨 문체 가이드 (Style Guide)**: 소설의 문장 스타일(어투, 톤앤매너, 금지/권장 어휘, 특정 작가/인물 모방 타깃 등)은 개별 작품 폴더 내의 `[Active Work Path]settings/style-guide.md` 파일 또는 시리즈 바이블의 `## Style Guide` 섹션에 선언되어 중앙 관리됩니다. 루터 에이전트와 `@novelist-loremaster`는 이 설정을 자동으로 상속하여 다른 서브 에이전트들에게 전파하므로, 매번 명령어를 보낼 때마다 문체를 지정하지 않아도 해당 작품에 소속된 모든 권수와 문단은 **항상 일관된 톤앤매너와 어조로 집필 및 퇴고**됩니다.
