...
| `/novelist-designer` | 개발 편집: 캐릭터, 세계관, 플롯 아이디어를 집필 가능한 구체 설계로 다듬습니다. |
...
```
   ...
   ② 설정 기록가 → 설정 데이터 및 최근 서사 상태 수집 (설정만 수집, 문체는 관여 안 함)
         │
   ③ Designer → 필요할 때 캐릭터/세계관/플롯 설계 구체화
         │
...
- **씨앗-열매 서사 성장**: 초고는 고립된 문장부터 시작하지 않습니다. 라우터는 먼저 씨앗을 기록하고, 추상적인 아이디어는 Designer로 캐릭터/세계관/플롯 설계를 구체화한 뒤, Length Budget이 붙은 Macro Skeleton이라는 가지를 만들고, Execution Unit Queue를 세운 뒤, 기존 피드백 루프로 잎을 붙이고, 검토로 꽃을 피우며, 검증과 산출물 갱신이 끝난 뒤에만 열매를 전달합니다.
...
- **Designer 개발 패스**: Designer는 넓은 아이디어를 집필 가능한 설계 제약으로 바꿉니다. 이 산출물은 라우터 기록과 설정 검증가 검토 전까지 임시 설계이며, 편집자의 미시 퇴고나 설정 검증가의 검증을 대체하지 않습니다.
...
/novelist-designer 주인공과 마법학교 세계관을 집필 가능한 설계로 구체화해 줘.
...
`novelist-writer`, `novelist-designer`, `novelist-editor`를 직접 호출한 결과는 임시 산출물입니다. 출력에는 반드시 `UNVERIFIED DRAFT`, `DESIGN DRAFT`, 또는 `UNVERIFIED REVISION` 상태가 표시되어야 하며, 라우터가 기록하고 필요한 경우 최종 `@novelist-otaku` 검증을 통과시켜 `verification-manifest.md`에 통과를 기록하기 전까지는 정식 원고, 캐논 설정, 출판 가능 원고, 적용 가능한 수정본으로 취급하지 않습니다.
